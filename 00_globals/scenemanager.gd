...
func change_scene_to_level(scene_path: String, target_node_name: String) -> void:
...
func change_scene_to_overworld(scene_path: String, target_node_name: String) -> void:
	scene_path_string = scene_path
	target_transition = target_node_name
	get_tree().change_scene_to_file(scene_path)
	await get_tree().process_frame
